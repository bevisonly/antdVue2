...
export default {
  ...
  methods: {
    ...
    renderSubMenu: function (h, menu) {
      let this_ = this
      let subItem = [h('span', { slot: 'title', attrs: { id: 'subItemSpan', style: 'width:80px;overflow:hidden;white-space:normal;text-overflow:clip;' } },
        [
          this.renderIcon(h, menu.meta ? menu.meta.icon : 'none', menu.fullPath),
          this.$t(getI18nKey(menu.fullPath))
        ]
    ...
    getSelectedKeys() {
      ...
      const highlight = current?.meta?.highlight
      if (highlight) {
        const resolve = this.$router.resolve({ path: highlight })
        matches = (resolve.resolved && resolve.resolved.matched) || matches
