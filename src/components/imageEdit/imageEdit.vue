<template>
  <a-modal
    ...
    :dialogStyle="{ top: suitStyle ? '16px' : '60px', paddingBottom: '0' }"
    :width='suitStyle ? (moreWidther || getWindowWidthHeight().windowContentWidth) : "70%"'
    @cancel="$emit('close')"
  >
    ...
    <template slot="footer">
      ...
      <div class="ctr-box">
        ...
        <a-button v-show="suitStyle"  @click="moreWidth" title="更宽">
...
export default {
  ...
  mounted() {
    ...
    this.suitStyle && (this.moduleCenter.height = `${this.getWindowWidthHeight().windowContentHeight}px`)
  },
  methods: {
    getWindowWidthHeight() {
      let windowContentHeight = (window.innerHeight || document.documentElement.clientHeight || document.body.clientHeight) - 201
      let windowContentWidth = window.innerWidth || document.documentElement.clientWidth || document.body.clientWidth
      windowContentHeight > 839 && (windowContentHeight = 839)
      windowContentHeight < 399 && (windowContentHeight = 399)
      windowContentWidth = windowContentWidth * .618
      return {windowContentHeight, windowContentWidth}
    },
    moreWidth() {
      this.widther = !this.widther
      this.moreWidther = this.widther ? '90%' : false
    },
    trimSpace(array) {
    ...
    start(e) {
      ...
      if (e.button == 0) {
        ...
        //获取需要拖动节点的坐标
        this.offset_x =
          document.getElementsByClassName("originStyle")[0].offsetLeft; //x坐标
        this.offset_y =
          document.getElementsByClassName("originStyle")[0].offsetTop; //y坐标
        //获取当前鼠标的坐标
