    ...
    <div :class="['header-bar', size]">
      ...
      <div class="btnList">
        <template v-if="btnList">
          <div>
            <a-button-group>
             <a-button :disabled='item.disabled' v-for="(item,index) in btnList" :key="index" :type="item.type||'primary'" icon='item.icon' @click="btnListClick(item.name)" >
              {{item.name}}</a-button>
            </a-button-group>
          </div>
        </template>
      ...
      <div class="actions">
        ...
        <a-tooltip title="列配置">
          <action-columns :searchShow='searchShow' :columns="columns" @reset="onColumnsReset" class="action">
            <template :slot="slot" v-for="slot in slots">
              <slot :name="slot"></slot>
            </template>
          </action-columns>
