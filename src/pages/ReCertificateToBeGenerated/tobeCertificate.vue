      ...
      >
        <a-row>
          <a-space>
            <a-button
              type="primary"
              @click="documentsGenerated"
              :loading="btnLoading"
              :disabled="
                !hasPermission('receivable_wait_certificated_generate')
              "
              >生成凭证</a-button
            >
            <a-button
              ...
              :disabled="!hasPermission('receivable_wait_certificated_export')"
              >导出</a-button
            >
          </a-space>
        </a-row>
        ...
        >
          ...
          <div slot="opreation" slot-scope="text, record">
            <a-button
              ...
              :disabled="!hasPermission('receivable_wait_certificated_invalid')"
              >作废</a-button
            >
            <a-button
              type="link"
              @click="toDetails(record)"
              :disabled="!hasPermission('receivable_wait_certificated_details')"
              >详情</a-button
            >
          </div>
...
import {
  ...
  InvalidOrder,
  exportData,
} from "../../services/settlement/receive/ReCertificateToBeGenerated";
...
const columns = [
  ...
  //   align: "center",
  //   title: "是否退货",
  //   dataIndex: "returnedState",
  //   width: 100,
  //   scopedSlots: { customRender: "returnedState" },
  // },
  {
    align: "center",
    title: "状态",
...
export default {
  ...
  methods: {
    ...
    downloadTem(url) {
      ...
      link.setAttribute("download", "应收-待生成凭证表");
    ...
    documentsGenerated() {
      ...
      if (this.selectedRows.length <= 0) {
        this.$message.error("请至少选择一条数据生成凭证");
        return;
      }
      let returnedState = false;
      let customerId = this.selectedRows[0].customerId;
      let customerState = false;
      this.selectedRows.forEach((item) => {
        console.log(item.returnedState);
        if (item.returnedState) {
          returnedState = true;
          return;
        }
      ...
      if (returnedState && this.selectedRows.length > 1) {
        console.log(returnedState);
        this.$confirm({
          title: "有退货的数据，是否增加一条退货凭证 ? ",
          onOk() {
            console.log("OK");
            that.createDocs();
            return;
      ...
      if (customerState && this.selectedRows.length > 1) {
        console.log(returnedState);
        this.$confirm({
          title: "有多个客户的数据，确定后将按照客户分别生成凭证 ",
          onOk() {
            console.log("OK");
            that.createDocs();
            return;
