          ...
          <a-col :span="24">
            ...
            <a-form-model-item class="formItemStyle">
              ...
              <a-button
                ...
                @click="exportData"
                :disabled="!hasPermission('reportSuppliersQuota_export')"
                >导出</a-button
              >
            </a-form-model-item>
          </a-col>
...
import {
  op,
  getLimitationSupplier,
  getSupplierList,
  exportLimitationSupplier
} from '@/services/report/reportSuppliersQuota'
...
const columns = [
  ...
  { title: '供应商等级', dataIndex: 'rating', width: 200, align: 'center' },
  {
    title: '评估额度/万元',
  ...
  methods: {
    exportData() {
      const params = {
        ...
        supplierIds: this.form.supplierId,
        orgIds: this.form.orgId
      }
      this.loadingExcel = true
      exportLimitationSupplier(params)
        .then(res => {
    ...
    submitPagination() {
      ...
      this.loading = true
      getLimitationSupplier(this.copyParams)
        .then(res => {
          if (res.data.code == '200') {
            this.loading = false
            this.pagination.total = res.data.totalNum
    ...
    handleOrgIdOption(value) {
      ...
      this.getSupplierListData(params)
    },
    ...
    getSupplierListData(params) {
      getSupplierList(params).then(
        res => (this.option.supplierOption = res.data || [])
      )
    },
    deepClone(obj) {
