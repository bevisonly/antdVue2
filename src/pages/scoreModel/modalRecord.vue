<template>
  <a-modal class="modalRecordTop" title="轨迹" :dialogStyle="{'top': '30px'}" v-model="visibleLModal" :maskClosable="false" :footer="null">
      ...
      <div class="flex-ct contentBox">
        <a-timeline>
          <a-timeline-item v-for="(item, i) in recordData" :key="i">
            ...
            <span class="commonStyle blueCursor" @click="detailsBtn(item.id)">评分详情</span>
          </a-timeline-item>
        </a-timeline>
      </div>
      <div class="flex-ed">
...
<script>
import { searchTrack as search } from '@/services/scoreCard/scoreModel'
export default {
  name: "modalRecord",
  data() {
    return {
  ...
  methods: {
    search(modelId, partnerId) {
      search({page:1, rows: 10, modelId, partnerId}).then(res => {
        if (res.data.total > 0) {
          ...
          this.$message.error("查看记录失败")
        }
      })
    },
    detailsBtn(id) {
      this.visibleLModal = false
      this.funDetailsBtn(id)
    },
    submitPagination() {
      let params = {
    ...
    openRecordModal(modelId, partnerId, fun) {
      this.pagination.page = 0
      this.pagination.size = 10
      this.modelId = modelId
      this.partnerId = partnerId
      this.funDetailsBtn = fun
