<template>
  <div class="new-page" :style="`min-height: ${pageMinHeight}px`">
    ...
    <div class="table-item">
      ...
      >
        <!-- :disabled="!hasPermission('to_be_warehoused_order_add')" -->
        <div class="btn-bar" style="margin-bottom: 16px; text-align: left">
          <div class="right-btn">
            <a-button
              type="primary"
              @click="addOrder"
              :disabled="!hasPermission('for_outbound_list_add')"
              >新增</a-button
            >
          </div>
          <div class="right-btn">
            <a-button-group>
              <a-button
                ...
                icon="printer"
                @click="printOrder"
                :disabled="!hasPermission('for_outbound_list_print')"
              >
                打印出库单
              </a-button>
              <a-button
                ...
                icon="download"
                @click="exportItem"
                :disabled="!hasPermission('for_outbound_list_export')"
              >
                导出订单
              </a-button>
            </a-button-group>
        ...
        >
          <div slot="imItemTypeName" slot-scope="text">
            <a-tag>{{ text }}</a-tag>
            <!-- <a-tag v-if="text == 10">销售单出库</a-tag>
            <a-tag v-if="text == 13">换货单出库</a-tag>
            <a-tag v-if="text == 103">换货入库</a-tag>
            <a-tag v-if="text == 90">领料出库</a-tag>
            <a-tag v-if="text == 91">盘亏出库</a-tag>
            <a-tag v-if="text == 98">其他出库</a-tag>
            <a-tag v-if="text == 12">退货出库</a-tag> -->
          </div>
          <div slot="opreation" slot-scope="text, record">
            <a-button
              type="link"
              @click="showDetails(record)"
              :disabled="!hasPermission('for_outbound_list_detail')"
              >查看订单</a-button
            >
            <a-button
              type="link"
              @click="returnWH(record)"
              :disabled="!hasPermission('for_outbound_list_outbound')"
              v-if="record.imItemType != 12"
              >出库</a-button
            >
            <!-- <a-popconfirm
              title="确定要删除这条数据吗?"
      ...
      <Details ref="details" />
      <AddWhOrder ref="addWhOrder" />
      <ReturnWh ref="returnWh" />
...
import AddWhOrder from "./addWhOrder.vue";
import ReturnWh from "./returnWh.vue";
...
export default {
  mixins: [mixin],
  components: { Details, AddWhOrder, ReturnWh, PrintOrder },
  data() {
    return {
  ...
  methods: {
    ...
    returnWH(record) {
      this.$refs.returnWh.showModal(record, this.warehouseList);
    },
    warehousing(record) {
      const params = {
        ...record,
      ...
      WarehousingConfirm(params).then((res) => {
        ...
        if (data.code == 200) {
          ...
          this.$message.success(
            data.message == "OK" ? "入库成功" : data.message
          );
        } else {
          this.$message.error(data.message ? data.message : "入库失败");
        }
      });
    },
    // delOrder(record) {
    ...
    //       this.getList();
    //       this.$message.success(
    //         res.data.message == "OK" ? "删除成功" : res.data.message
    //       );
    //     } else {
    ...
    // },
    addOrder() {
      this.$refs.addWhOrder.showModal(this.warehouseList);
    },
    ...
    downloadTem(url) {
      ...
      link.setAttribute("download", "待出库单");
