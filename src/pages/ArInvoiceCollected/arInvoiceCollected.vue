         ...
         -->
          ...
          >
            ...
            <span slot="state" slot-scope="{ text }">
              <a-tag>{{ stateTxt[text] }}</a-tag>
            </span>
            <template slot="operation" slot-scope="{ record }">
              <div>
                <a-button type="link" @click="toDetails(record)" :disabled='!hasPermission("collected_detail")'>详情</a-button>
                <!-- 202210271558yyb 隐藏 -->
                <a-popconfirm
                  @confirm="orderRecall(record)"
                  title="确定撤回吗?"
                  :disabled='!hasPermission("collected_revoke")'
                >
                  <a-button type="link" :disabled='!hasPermission("collected_revoke")' >撤回</a-button>
                </a-popconfirm>
              </div>
            </template>
            <template slot="invoiceMa" slot-scope="{ record }">
              <div>
                <a-button
                  type="link"
                  @click="toInvoiceInfo(record)"
                  v-if="record.issueState == 0 || record.issueState == 5"
                  :disabled='!hasPermission("collected_invoice")'
                  >开票</a-button
                >
                <a-button
                  type="link"
                  @click="toInvoiceInfo(record)"
                  v-if="record.issueState == 1 || record.issueState == 3||record.issueState==4"
                  :disabled='!hasPermission("collected_invoice")'
                  >领票</a-button
                >
                <a-popconfirm @confirm="redAction(record)" title="确定红冲吗?" :disabled='!hasPermission("collected_reverse")'>
                  <a-button type="link" style="color: red" :disabled='!hasPermission("collected_reverse")'>红冲</a-button>
                </a-popconfirm>
                <a-button
                  type="link"
                  style="color: blue"
                  @click="blueBrother(record)"
                  :disabled='!hasPermission("collected_compensate")'
                  >蓝补</a-button
                >
              </div>
            </template>
...
import { RevokeSettlement } from "../../services/settlement/receive/collected";
import { redGo } from "@/services/settlement/receive/clearingAccountsNeedget";
const columns = [
  {
    align: "center",
    title: "结算单号",
  ...
  { align: "center", title: "发票代码", dataIndex: "invoiceCode", width: 150 },
  {
    align: "center",
    title: "发票号码",
  ...
  },
  {
    align: "center",
    title: "收款金额",
    dataIndex: "receiptsAmount",
    width: 120,
  },
  {
    align: "center",
    title: "未收金额",
    dataIndex: "unReceiptsAmount",
    width: 120,
  },
  {
    align: "center",
  ...
  },
  {
    ...
    dataIndex: "invoiceMa",
    width: 200,
    scopedSlots: { customRender: "invoiceMa" },
  },
];
export default {
  name: "invoiceList",
  mixins: [mixin],
  components: { advanceTable, InvoiceApply },
  data() {
    return {
      exportLoading: false,
      selectedRowKeys: [],
      ...
      opList: [],
      searchState: true,
      btnList: [{ name: "导出列表", disabled: false }],
      orgId: "",
      storeList: [],
      stateTxt: ["", "未收款", "部分收款", "已收款", "已核销"],
      footerTotal: [
        { name: "单据金额", value: "totalAmount" },
        { name: "税额", value: "taxAmount" },
        { name: "不含税金额", value: "includingTaxAmount" },
        { name: "扣点金额", value: "deductionAmount" },
        { name: "应收金额", value: "receivableAmount" },
        { name: "包装金额", value: "packTotalAmount" },
      ],
    };
  },
  activated() {
    ...
    this.getOp();
    this.$setPageTitle(
      "/balance/payable/apInvoiceListInvoice",
      "应付-发票管理"
    );
  },
  created() {
    this.btnList.forEach((item) => {
      if (item.name == "导出列表" && !this.hasPermission("collected_export")) {
        item.disabled = true;
      }
    });
  },
  methods: {
    redAction(record) {
      if (record.issueState != 1 && record.issueState != 3) {
        this.$info({
          title: "提示",
          content: "该单据还未全部开票或未蓝补，请先全部开票或蓝补！",
          okText: "关闭",
        });
        return;
      }
      redGo({ id: record.id || "" })
        .then((res) => {
          if (res.data.code == 200) {
            this.$message.success(res.data.message || "红冲成功");
            this.getList();
          } else {
    ...
    //蓝补操作
    blueBrother(record) {
      if (record.issueState != 2) {
        this.$info({
          title: "提示",
          content: "该单据还未红冲，请先红冲！",
          okText: "关闭",
        });

        return;
      }
      this.$refs.invoiceApply.showModal(2, record,'',true);
    },
    //已结算单撤销
    orderRecall(record) {
      RevokeSettlement({ id: record.id })
        .then((res) => {
          if (res.data.code == 200) {
            this.$message.success(res.data.message || "撤回成功");
            this.getList();
          } else {
            this.$message.error(res.data.message);
          }
    ...
    toDetails(record) {
      this.$router.push({
        path: "/balance/receiveable/arInvoiceCollectedDetails",
        query: {
          id: record.id,
          arInvoiceCode: record.arInvoiceCode,
        },
    ...
    downloadTem(url) {
      ...
      link.setAttribute("download", "应收-已收账款列表");
