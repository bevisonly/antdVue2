      ...
      <div class="table">
        ...
        <div class="table-data">
          <a-table
            ...
            :scroll="{ y: ItemData.length > 10 ? 800 : 0 }"
            :row-selection="{
              columnWidth: 50,
  ...
  methods: {
    ...
    handleSubmitItems() {
      ...
      this.rowSelectionChangeData.forEach((item) => {
        item.vat = item.outputTaxRate ? item.outputTaxRate : "";
        if (this.resStrData.length) {
          this.resStrData.forEach((val) => {
            if (item.itemName === val.name) {
              item.saleQty = val.num;
              item.priceUnit = val.unit;
            }
          });
        }
      });
      this.$parent.$parent.getItems(this.rowSelectionChangeData, "items");
    },
    ...
    openPrudoct(customerId, orderDate) {
      ...
      componentGetItems(params).then((res) => {
        ...
        if (data.code == 200) {
          ...
          this.ItemData.forEach(item=>{
            item.spec=item.spec?item.spec:(item.specList.length===1?item.specList[0].specName:'')
          })
