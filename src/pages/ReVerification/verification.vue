        ...
        >
          <div style="margin-bottom: 16px; text-align: left" class="flex-sb">
            <a-space>
              <a-button
                ...
                :disabled="
                  resourceControl(
                    pageState === 'receivableWaitVerification'
                      ? 'receivable_wait_verification_export_list'
                      : 'receivable_verification_export_list'
                  )
                "
              >
                导出列表
              ...
              <a-button
                v-if="pageState === 'receivableWaitVerification'"
                type="primary"
                icon="appstore"
                @click="checkMore"
                :disabled="
                  resourceControl('receivable_wait_verification_check')
                "
              >
                批量核销
              </a-button>
          ...
          <a-spin :tip="spinText" :spinning="spinning">
            ...
            >
              ...
              <div slot="settleCycleType" slot-scope="text">
              ...
              <div slot="operation" slot-scope="text, record">
                <a-button
                  ...
                  v-if="pageState === 'receivableWaitVerification'"
                  :disabled="
                    !hasPermission('receivable_wait_verification_verificate')
                  "
                  >核销</a-button
                >
                <a-button
                  type="link"
                  @click="toDetails(record)"
                  v-if="pageState === 'receivableVerification'"
                  :disabled="!hasPermission('receivable_verification_details')"
                  >详情</a-button
                >
              </div>
              <div slot="export" slot-scope="text, record">
                <a-button
                  ...
                  :disabled="
                    resourceControl(
                      pageState === 'receivableWaitVerification'
                        ? 'receivable_wait_verification_export_details'
                        : 'receivable_verification_export_details'
                    )
                  "
                  >导出明细</a-button
                >
              ...
              <div slot="footer" class="table-footer">
                本页合计 :
                <span
                  >收款金额<span>{{ receivableAmountTo }}</span></span
                >|<span
...
import { getOpSubject, GetOpList } from "../../services/signed/signed";
...
const columns = [
  {align: "center",title: "结算日期",dataIndex: "settlementDate",width: 150,},
  {align: "center",title: "核销状态",dataIndex: "state",width: 100,scopedSlots: { customRender: "state" },},
  {align: "center",title: "单据编号",dataIndex: "arInvoiceCode",width: 150,},
  {align: "center",title: "核销类型",dataIndex: "dataType",width: 150,scopedSlots: { customRender: "dataType" }},
  {align: "center",title: "银行流水",dataIndex: "relatedBankCode",width: 150},
  {align: "center",title: "客户",dataIndex: "customerName",width: 250,},
  {align: "center",title: "门店名称",dataIndex: "storeName",width: 150,},
  {align: "center",title: "结款人",dataIndex: "payee",width: 100,},
  {align: "center",title: "结款电话",dataIndex: "payeePhone",width: 150,},
  {align: "center",title: "结算周期",dataIndex: "settleCycleType",width: 250,scopedSlots: { customRender: "settleCycleType" },},
  {align: "center",title: "税额",dataIndex: "taxAmount",width: 100,},
  {align: "center",title: "不含税金额",dataIndex: "includingTaxAmount",width: 150,},
  {align: "center",title: "结算金额",dataIndex: "totalAmount",width: 150,},
  {align: "center",title: "收款金额",dataIndex: "receivableAmount",width: 150,},
  {align: "center",title: "已开发票金额",dataIndex: "invoiceAmounted",width: 150,},
  ...
  //! glht
  {align: "center",title: "操作",dataIndex: "operation",width: 100,fixed: "right",scopedSlots: { customRender: "operation" },},
  {align: "center",title: "导出",dataIndex: "export",width: 100,fixed: "right",scopedSlots: { customRender: "export" },},
];
export default {
  mixins: [mixin],
  data() {
  ...
  activated() {
    ...
    switch (this.$route.path) {
      case "/balance/receiveable/receivableWaitVerification":
        this.pageState = "receivableWaitVerification";

        break;
      case "/balance/receiveable/receivableVerification":
        this.pageState = "receivableVerification";
        break;

      default:
        break;
    }
    if (this.pageState === "receivableWaitVerification") {
      let tempCol = [];
  ...
  methods: {
    ...
    handleDateChange(val) {
      this.searchform.settlementDateStart = val[0];
      this.searchform.settlementDateEnd = val[1];
    },
    ...
    exportItem() {
      ...
      if (this.pageState === "receivableVerification") {
        ExportList(params)
          .then((res) => {
            console.log(res);
            const data = res.data;
    ...
    checkMore() {
      ...
      this.$confirm({
        ...
        cancelText: "取消",
        async onOk() {
          checkMore1({ids: This.selectedRowKeys.join(',')}).then((res) => {
    ...
    exportDetails(record) {
      ...
      this.spinText = "数据导出中请稍后";
      const params = {
        arCode: record?.arInvoiceCode ?? "",
        totalAmount: record?.totalAmount ?? "",
        settleDate: record?.settlementDate ?? "",
        arState: record?.state ?? "",
        arInvoiceId: record?.id ?? "",
      };
    ...
    downloadTem(url, state) {
      ...
      if (state) {
        ...
        this.pageState === "receivableWaitVerification"
          ? link.setAttribute("download", "应收-待核销列表")
          : link.setAttribute("download", "应收-已核销列表");
      }
  ...
  computed: {
    ...
    getTotal() {
      ...
      this.selectedRowKeys.forEach((item) => {
        this.tableList.forEach((val) => { item == val.id && money.push(val.totalAmount) })
      })
