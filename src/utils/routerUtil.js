...
/**  
 新增前端控制路由。有些页面不用后台给权限，需要在前端写死。调用此方法
 * @param routeData 路由源数据
 * @param data 需要添加的路由数据。例 [{parentCode: "rate",routeChildrens: [{children: [],code: "add_output_rate",description: "uap菜单",id: "",meta: { href: "" },metadata: {},name: "新增销项税率",parentId: "",router: "addOutputRate",type: 2,url: "",},],},]
*/
function addChildRoutes(routeData, data) {
  routeData.forEach(item => {
    if (item.code) {
      let routeObj = data.find(val => {
        return item.code == val.parentCode
      })
      if (routeObj) {
        item.children.push(...routeObj.routeChildrens)
      } else {
        addChildRoutes(item.children, data)
      }
    }
  })
  return routeData
...
export { parseRoutes, loadRoutes, formatAuthority, getI18nKey, loadGuards, deepMergeRoutes, formatRoutes, setAppOptions, addChildRoutes }
